...
def collect_image_paths(data_dir):
    ...
    crawler_ = futil.FileSystemDataCrawler(data_dir, image_keys, MyFilePathGenerator(), dir_filter, '.nii.gz')
...
def load_images(image_paths):
    ...
    #T1 = next(iter(image_paths.values()))
    #GroundTruth = next(iter(image_paths.values()))

    image_dict = {
        #structure.BrainImageTypes.T1w: sitk.ReadImage(T1, sitk.sitkFloat32),
        #structure.BrainImageTypes.GroundTruth: sitk.ReadImage(GroundTruth, sitk.sitkUInt8)
        structure.BrainImageTypes.T1w: sitk.ReadImage(image_paths.__getitem__(structure.BrainImageTypes.T1w), sitk.sitkFloat32),
        structure.BrainImageTypes.GroundTruth: sitk.ReadImage(image_paths.__getitem__(structure.BrainImageTypes.GroundTruth), sitk.sitkUInt8)
...
def register_images(image_dict, atlas_img):
    ...
    #registered_t1 = registration.execute(image_dict.get(structure.BrainImageTypes.T1w), registration_params)
